/**
 * ChallengeMate — Google Apps Script backend (your Google Sheet IS the database).
 *
 * Setup
 *  1. Create a Google Sheet (any name, e.g. "ChallengeMate DB").
 *  2. Extensions ▸ Apps Script ▸ delete the default code ▸ paste this whole file.
 *  3. Deploy ▸ New deployment ▸ type "Web app":
 *       - Description: ChallengeMate API
 *       - Execute as: Me
 *       - Who has access: Anyone
 *  4. Authorize when prompted (your Google account only).
 *  5. Copy the Web App URL (ends in /exec) into the frontend:
 *       src/services/googleSheets.ts  →  SHEETS_API_URL
 *  6. Commit & push — GitHub Actions publishes the site to GitHub Pages.
 *
 * On first run this script creates the 11 tabs + headers (README §23 schema).
 *
 * IMPORTANT: after every code change you must create a NEW deployment version
 * (Deploy ▸ Manage deployments ▸ pencil ▸ Version: "New version"), otherwise
 * the /exec URL keeps serving the old code.
 */
...
/* Sheets auto-converts "2025-01-05" cells to Date objects; normalize on read. */
function key_(d){ return Utilities.formatDate(d, Session.getScriptTimeZone(), 'yyyy-MM-dd'); }
function dateStr_(v){
  if(v instanceof Date) return key_(v);
...
function rows(name){
  ...
  for(var i=1;i<v.length;i++){
    ...
    for(var j=0;j<head.length;j++){
      var cell = v[i][j];
      r[head[j]] = cell instanceof Date ? key_(cell) : cell;
    }
...
/* Challenges store rules as one-per-line text; numbers may come back as strings. */
function normChallenge_(c){
  if(!c) return c;
  c.start_date = dateStr_(c.start_date);
  c.end_date   = dateStr_(c.end_date);
  c.created_at = dateStr_(c.created_at);
  c.penalty_points = Number(c.penalty_points || 0);
  c.main_vacation_day = Number(c.main_vacation_day === '' ? 5 : c.main_vacation_day);
  c.optional_vacations_per_week = Number(c.optional_vacations_per_week === '' ? 0 : c.optional_vacations_per_week);
  c.rules = Array.isArray(c.rules) ? c.rules : String(c.rules || '').split(/\r?\n/).filter(Boolean);
  return c;
}

function getChallenge_(id){
  var c = rows('Challenges');
  for(var i=0;i<c.length;i++) if(c[i].challenge_id === id) return normChallenge_(c[i]);
  return null;
}
...
function usernameOf_(uid){
  var u = rows('Users');
  for(var i=0;i<u.length;i++) if(u[i].user_id === uid) return u[i].username;
  return 'unknown';
}

/* Daily task state machine (README §11–12): stored row wins, else derive. */
...
function computeStats_(ch, uid){
  ...
  var comps = rows('Completions'); var compByTask = {};
  comps.forEach(function(c){ compByTask[c.task_id] = c; });
  var today = today_();
  ...
  /* Penalties sheet is an event log: +points for misses are derived,
     stored rows are negative reductions (README §14). */
...
function optLeft_(ch, uid){
  ...
  var today = today_();
  var start = addDays_(today, -((weekday_(today)+6)%7)); /* Monday */
  var week = {}; range_(start, addDays_(start,6)).forEach(function(k){ week[k] = 1; });
  var used = rows('Vacations').filter(function(v){
    return v.challenge_id === ch.challenge_id && v.user_id === uid && v.type === 'optional' && week[v.date];
  }).length;
...
function handle_(action, p){
  ...
  if(action === 'register'){
    ...
    addRow('Users', nu);
    addRow('NotificationSettings', { user_id:nu.user_id, reminders_enabled:false, reminder_frequency:2,
      reminder_start:'09:00', reminder_end:'22:00' });
    return nu;
  }
  ...
  if(action === 'enroll'){
    addRow('Members', { challenge_id:p.challengeId, user_id:p.userId, role:'member',
  ...
  if(action === 'completeTask'){
    ...
    if(p.duration != null || p.summary)
      addRow('Completions', { task_id:taskId, duration:p.duration != null ? p.duration : '', summary:p.summary || '' });
  ...
  if(action === 'vacation'){
    ...
    addRow('DailyTasks', { task_id:taskId2, challenge_id:p.challengeId, user_id:p.userId, date:today3,
      status:'VACATION', completed_at:'' });
    addRow('Vacations', { vacation_id:'v_'+p.userId+'_'+today3+'_'+new Date().getTime(),
      challenge_id:p.challengeId, user_id:p.userId, date:today3, type:'optional' });
  ...
  if(action === 'detail' || action === 'progress'){
    ...
    var lb = visible.map(function(m){
      var s = computeStats_(ch, m.user_id);
      return { user_id:m.user_id, username:usernameOf_(m.user_id), anonymous:false, streak:s.currentStreak,
        best:s.bestStreak, done:s.done, penalties:s.penalties, completionPct:s.completionPct,
    ...
    if(action === 'progress'){
      ...
      var statuses = {};
      range_(ch.start_date, ch.end_date < today_() ? ch.end_date : today_()).forEach(function(k){
        statuses[k] = statusFor_(k, ch, byDate2);
      });
    ...
    var weekArr = range_(monday, addDays_(monday,6)).map(function(k){
      return { key:k, label:WEEKD[weekday_(k)], dayNum:Number(String(k).slice(8)),
        ...
        isToday:k === todayW, isMain:weekday_(k) === Number(ch.main_vacation_day) };
    });

    /* penalty event log: derived +events for misses, stored −events for reductions */
    var penEvents = []; var penBal = 0;
    if(mine){
      var removals = rows('Penalties').filter(function(x){
        return x.challenge_id === p.challengeId && x.user_id === p.userId;
      });
      ...
      removals.forEach(function(r){
        penEvents.push({ date:r.date, reason:String(r.reason || 'Penalty removed'),
          points:Number(r.points), removable:false });
        penBal += Number(r.points);
      });
    ...
    return { challenge:ch, member:mine,
      participants:visible.map(function(m){ return { user_id:m.user_id, username:usernameOf_(m.user_id),
        role:m.role, isYou:m.user_id === p.userId }; }),
      hiddenCount:mem.length - visible.length,
      myStats:mine ? computeStats_(ch, p.userId) : null,
      week:weekArr, leaderboard:lb, iAmAnonymous:mine ? !!mine.anonymous : false,
      penalties:{ events:penEvents, balance:penBal },
      optLeft:mine ? optLeft_(ch, p.userId) : 0 };
  }

  if(action === 'profile'){
    u = rows('Users'); var me = null;
    for(i=0;i<u.length;i++) if(u[i].user_id === p.userId) me = u[i];
    if(!me) return { error:'User not found.' };
    return { user:me,
      global:{ currentStreak:0, bestStreak:0, done:0, missed:0, vacations:0, totalMinutes:0,
        completionPct:0, applicableDays:0, perfectWeeks:0, maxDayMinutes:0, comeback:false, penalties:0 },
      achievements:[],
      settings:{ user_id:p.userId, reminders_enabled:false, reminder_frequency:2,
        reminder_start:'09:00', reminder_end:'22:00' },
      friendPrefs:[],
      challengeCount:rows('Members').filter(function(m){ return m.user_id === p.userId; }).length };
  }
  ...
  if(action === 'decreasePenalty'){
    ch = getChallenge_(p.challengeId);
    addRow('Penalties', { penalty_id:'p_'+new Date().getTime(), challenge_id:p.challengeId, user_id:p.userId,
      date:today_(), points:-(Number(ch ? ch.penalty_points : 1)), reason:'Penalty decreased', created_at:today_() });
    return { ok:true };
  }

  if(action === 'createChallenge'){
    var cid = 'c_'+new Date().getTime();
    addRow('Challenges', { challenge_id:cid, name:p.name, icon:p.icon || 'book',
      description:p.description || '', rules:(p.rules || []).join('\n'), owner_id:p.userId,
      start_date:p.start_date, end_date:p.end_date, deadline:p.deadline || '23:00',
      penalty_points:p.penalty_points != null ? p.penalty_points : 1,
      main_vacation_day:p.main_vacation_day != null ? p.main_vacation_day : 5,
      optional_vacations_per_week:p.optional_vacations_per_week != null ? p.optional_vacations_per_week : 1,
      created_at:today_() });
    addRow('Members', { challenge_id:cid, user_id:p.userId, role:'owner', anonymous:false, joined_at:today_() });
    (p.inviteIds || []).forEach(function(uid){
      if(uid !== p.userId) addRow('Members', { challenge_id:cid, user_id:uid, role:'member',
        anonymous:false, joined_at:today_() });
    });
    return { challenge_id:cid };
  }

  if(action === 'listUsers')
    return rows('Users').map(function(x){ return { user_id:x.user_id, username:x.username }; });

  if(action === 'saveSettings' || action === 'saveFriendPrefs' || action === 'quote')
    return { ok:true };
...
/* Opening the /exec URL in a browser answers a ping — handy for smoke tests. */
